...
def tfc_to_als(machines, qubit_alloc, capacity, cost, tfc_path, als_path, force=False, condense_layers=True, cost_vm=False, info=False, verbose=False):
    ...
    for line in tfc.splitlines():
        ...
        if gate == 't' or gate == 'f':
            ...
            v_list = rem
            if info:
                circ_freq = defaultdict(lambda: {key: 0 for key in v_list})
   ...
   #int_bits = alloy_int_bits(tele_max)
    ...
    cost_to = cost_to_string(m_list, cost) if cost_vm else None
    ...
    als = Template(
        ALLOY_TEMPLATE_COST_VM if cost_vm else ALLOY_TEMPLATE_BASIC
        ).safe_substitute({
            'LOCATION': location,
            ...
            'COST_TO': cost_to,
            'VAC_MAX': machines - 1,        # NOTE heuristic
            'COST_MAX': cost * tele_max // 5    # NOTE heuristic
        })
...
def template_tele_int(als):
    als = re.sub(r'lte\[grph/last\.numTele, (\d+)\]|\brun finalLayer for (\d+) circGraph, (\d+) Int',
        lambda match:
            r'lte[grph/last.numTele, $TELE_MAX]' if match.group(1)
            else f'run finalLayer for {match.group(2)} circGraph, $INT_BITS Int',
        als, count=2)
    return Template(als)
...
if __name__ == '__main__':
    ...
    for filename in files:
        ...
        if ".als" in post_fix:
            gen_als = False
        elif ".tfc" in post_fix:
            gen_als = True
        else:
            print(f"\tNote: The given file {filename} is expected to be a '.tfc' or a '.als' file.", flush=flush_out)
